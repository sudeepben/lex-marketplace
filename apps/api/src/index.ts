...
function productDocRef(orgId: string, appId: string, id: string) {
  return db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("products").doc(id);
}
function offersCol(orgId: string, appId: string) {
  return db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("offers");
}
async function assertOwnerOrThrow(docRef: FirebaseFirestore.DocumentReference, uid: string) {
...
const BookmarkInput = z.object({ productId: z.string().min(1) });
...
const OfferUpdateInput = z.object({ status: z.enum(["accepted", "declined"]) });
...
app.post("/offers", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const productRef = productDocRef(orgId, appId, productId);
    const productDoc = await productRef.get();
    if (!productDoc.exists) return res.status(404).json({ error: "Product not found" });

    const sellerId = (productDoc.data() as any).ownerId as string;
    if (!sellerId) return res.status(400).json({ error: "Product missing ownerId" });
    if (sellerId === buyerId) return res.status(400).json({ error: "You cannot offer on your own product" });
    ...
    const docRef = await offersCol(orgId, appId).add(payload);
...
app.get("/offers", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    let q: FirebaseFirestore.Query = offersCol(orgId, appId);
    q = q.where(role === "buyer" ? "buyerId" : "sellerId", "==", uid);
    if (productId) q = q.where("productId", "==", productId);
    q = q.orderBy("createdAt", "desc"); // composite index required
...
app.post("/products", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const colRef = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("products");
...
app.post("/reviews", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const colRef = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("reviews");
    const payload = {
      productId,
      rating,
      text,
...
app.post("/bookmarks", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const colRef = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("bookmarks");
    ...
    const payload = { userId: uid, productId, createdAt: admin.firestore.FieldValue.serverTimestamp() };
    const docRef = await colRef.add(payload);
    return res.status(201).json({ id: docRef.id });
  } catch (err: any) {
...
app.delete("/bookmarks/:id", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const docRef = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("bookmarks").doc(id);
...
app.get("/bookmarks", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const prodCol = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("products");
    const prodDocs = await Promise.all(bookmarks.map((b) => prodCol.doc(b.productId).get()));
...
app.get("/products", async (req, res) => {
  try {
    ...
    // >>> IMPORTANT: if using price inequality, orderBy price first
...
app.get("/reviews", async (req, res) => {
  try {
    ...
    const colRef = db.collection("orgs").doc(orgId).collection("apps").doc(appId).collection("reviews");
    const snap = await colRef.where("productId", "==", productId).orderBy("createdAt", "desc").limit(pageSize).get();
...
app.put("/products/:id", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const updates = { ...parsed.data, updatedAt: admin.firestore.FieldValue.serverTimestamp() };
...
// Uploads
app.post("/upload", verifyFirebaseToken, upload.array("files", 5), (req: any, res) => {
  try {
